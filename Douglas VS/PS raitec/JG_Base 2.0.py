...
fruta = ["banana", "laranja", "maracuja", "limao"]
...
def desenho_da_forca(window, chances):
    background_image = pg.image.load("lousapreta.png").convert()  # escolhe a imagem
...
def Popup_Escolha(ganhou):
    layout_palavra = [ # popup com a palavra
        sg.popup_ok("A palavra era", title=("PALAVRA"))
    ]
    ...
    layout_perder = [  # Layout do botões caso perder
        [sg.Text(
            "Que pena, você perdeu. Deseja jogar novamente ?")],
        [sg.B('Sim !', button_color='Green'),
         sg.B('Não.', button_color='Red')],
    ]
